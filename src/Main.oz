...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   % Returns an empty dynamic map of M rows and N columns,
   ...
   % Returns a list of all the player of kind 'Kind' ('P' for Pacmans,
   % 'G' for Ghosts), whose elements are of type:
   % player(port:P id:<pacman>/<ghost>)
   ...
   % Spawns all the players and items on the GUI and
   % local dynamic map for the first time, and warns
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   % Executes the turn of the given player currently at
   % position CurPos, and sends adequate message to GUI,
   % Main and Objects ports.
   ...
   % Returns the spawn position of player with
   % <pacman>/<ghost> id 'PlayerID'
   ...
   % Launches the turn of the given Player currently at CurPos
   % on the map, and send adequate message to the Main's port.
   ...
   % Returns the <pacman> id of the winner
   fun {GetWinner}
      local Aux in
   ...
   % Sends a first ordered shot of messages from
...
%%%%%%%%%%%%%%%%%%%%%%%% 
   ...
   % Handles one given message H from StreamMain
...
%%%%% DECLARATIONS %%%%%
   ...
   HuntMode = {NewCell false}                    % Keeps track of Mode
   SpawnsP = {Tuple.make 's' Input.nbPacman}     % Spawns position of Ghosts
   SpawnsG = {Tuple.make 's' Input.nbGhost}      % Spawns positions of Pacmans
   Board = {CreateMap Input.nRow Input.nColumn}  % Local dynamic Map
   Alive = {NewCell Input.nbPacman}              % Number of Pacmans who still have lives left

   % Port objects
   {NewPort StreamMain PortMain}         % Main port
   {NewPort StreamPacmans PortPacmans}   % @AllPacmans port
   {NewPort StreamGhosts PortGhosts}     % @AllGhosts port
   {NewPort StreamBoard PortBoard}       % Port-object for the dynamic map
   {NewPort StreamObjects PortObjects}   % Port-object for other dynamic variables
...
%%%%% START GAME %%%%%
   ...
   % Initialize and spawn players/items on GUI & local dynamic map
